...
class image_loader(data.dataset.Dataset):
    ...
    def __getitem__(self, index):
        while(True):
            ...
            img_name, _ = get_img_file(self.data_dir, file_name, random.random())
        ...
        # label = label.transpose(2, 0)
        
        # fh, fw, t = label.shape
        # print("label", label.shape)
        
        if self.mode == "train":
            return face_vid, flipped_vid, label[:, :, 0]
            # return face_vid, flipped_vid, label
        else:
            return face_vid, label[:, :, 0]
...
def build_seq_dataset(args, mode="train", task="exp"):
    if mode == "train":
        ...
        dataset = image_loader(args, data_dir='./ABAW8', label_dir=args.train_csv_path, \
            frame_length=args.clip, mode="train", task=task)
    elif mode =="valid":
        print("Dataloader building... [Val set]")
        dataset = image_loader(args, data_dir='./ABAW8', label_dir=args.valid_csv_path, \
