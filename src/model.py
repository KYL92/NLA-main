...
class NLA_r18(nn.Module):
    ...
    def __init__(self, args):
        ...
        Resnet18 = resnet18()
        cp = torch.load('./src/resnet18_msceleb.pth')
...
class NLA_r50(nn.Module):
    ...
    def __init__(self):#, args):
        ...
        with open('./weights/resnet50_ft_weight.pkl', 'rb') as f:
            obj = f.read()
...
class ResNet50FeatureExtractor(nn.Module):
    def __init__(self):
        super().__init__()
        base_model = NLA_r50()
        cp = torch.load('./weights/r50_best_mean.pth')
...
class SwinTransformerFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        cp = torch.load('./weights/swin_best_mean.pth', map_location='cpu')
...
class VideoFeatureModel_static(nn.Module):
    def __init__(self, inc, base_model, temporal_model):
        ...
        self.classifier = nn.Linear(inc, 8)
    ...
    def forward(self, x):
        ...
        static_features, static_outputs = self.base_model(x)
        ...
        static_output = self.static_classifier(static_outputs)
...
class VideoFeatureModel_concat_3D(nn.Module):
    ...
    def forward(self, x, is_concat=False):
        ...
        if is_concat:
            batch, c, clip_length, h, w = x.size()
            half_clip = clip_length // 2  # clip_length를 절반으로 나누기
            x, flipped_x  = torch.split(x, half_clip, dim=2)
            #  (batch, channel, clip, 224, 224)
            static_features = self.base_model(x)
            flipped_static_features = self.base_model(flipped_x)
            # batch, 8, 32
            
            # Temporal processing separately
            output_1 = self.temporal_model(static_features)
            output_2 = self.temporal_model(flipped_static_features)
...
class VideoFeatureModel(nn.Module):
    def __init__(self, base_model, temporal_model):
        super().__init__()
        self.base_model = base_model  # ResNet50 기반 feature extractor
        self.temporal_model = temporal_model  # Temporal Transformer or RNN model
...
class VideoFeatureModel_concat(nn.Module):
    def __init__(self, args, base_model, temporal_model):
        super().__init__()
        self.base_model = base_model  # ResNet50 기반 feature extractor
        self.temporal_model = temporal_model  # Temporal Transformer or RNN model
    ...
    def forward(self, x, is_concat=False):
        ...
        if is_concat:
            ...
            # Classifier 적용
            output_1 = self.classifier(output_1)
            output_2 = self.classifier(output_2) # [B, t, ch]
            
            # output_1 = output_1.flatten(1)  # [B, t*ch]
            # output_2 = output_2.flatten(1)  # [B, t*ch]
            
            output_1 = self.final_classifier(output_1)
            output_2 = self.final_classifier(output_2)
